  ...
  setup() {
    ...
    const minifyImage = (fileobj: File) => {
      ...
      function photoCompress(
        file: File,
        w: fileQuality,
        objDiv: (base64Codes: string) => void
      ) {
      ...
      function canvasDataURL(
        path: string,
        obj: fileQuality,
        callback: (base64Codes: string) => void
      ) {
        var img = new Image();
        img.src = path;
        img.onload = function () {
          ...
          // const bl = convertBase64UrlToBlob(base64)
          // console.log(bl.size,99999);
          let base64=''
          function checkImageSize(quality:number){
            base64 =canvas.toDataURL("image/jpeg", quality)
            const bl=convertBase64UrlToBlob(base64)
            if(bl.size>1024*1024){
              checkImageSize(quality*0.7)
            }
          }
          checkImageSize(quality)
          // 回调函数返回base64的值
          callback(base64);
        };
      }
      ...
      if (file.size > 1024 * 1024) {
        photoCompress(file, { quality: 0.7 }, (base64Codes) => {
          const bl = convertBase64UrlToBlob(base64Codes);
          const file2 = new File(
            [bl],
            "file_" + new Date().getTime() + ".jpg",
            { type: bl.type }
          );
          upload(file2);
